...
Note the +DbPurge.start+ method takes an optional parameter that indicates the
tableset that the tests use and thus the tables that will be cleaned. If unspecified
the +:default+ tableset is used.

== Tests that use a transaction

A test can indicate that it uses a transaction by invoking +DbPurge.uses_transaction+
between the invocation of +DbPurge.start+ and +DbPurge.finish+. This will ensure that
the next test attempts to clean the database.

== Configuring the clean action

The clean action attempts to issue a +DELETE FROM+ SQL statement for every table in the
tableset supplied to the +DbPurge.start+ method. The tableset also defines the order
